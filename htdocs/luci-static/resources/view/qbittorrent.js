...
function randomToken(e, id) {
	var q = '[%s="%s"]'.format('id', id);
	e.querySelector(q).value = Math.floor( Math.random() * (65535 - 1024)) + 1024;
};
...
return L.view.extend({
	load: function() {
		return fs.exec('/usr/bin/qbittorrent-nox', ['-v'], {'HOME': '/var/run/qbittorrent'}).then(function(res) {
			...
			return L.toArray(res.stdout)[1] || '';
		});
	},

	renderBtn: function(m, eid) {
		var e = m.findElement('id', eid), oe, ne, str;
		if(e) {
			if(oe = e.nextElementSibling) {
				str = oe.textContent,
				ne = E('button', {'class': 'cbi-button cbi-button-neutral'}, str);
				ne.onclick = randomToken.bind(this, e, 'widget.' + eid);
				L.dom.content(oe, ne);
			}
		}
	},
	...
	render: function(v) {
		...
		o = s.taboption('connection', form.Value, 'PortRangeMin', _('Connection Port'), _('Generate Randomly'));
		o.depends('UseRandomPort', 'false');
		o.datatype = 'range(1024,65535)';
		o.cbid = function(section_id) {
			eid = this.super('cbid', section_id);
			return eid;
		}
		...
		o = s.taboption('bittorrent', form.Flag, 'uTP_rate_limited', _('μTP Rate Limit'),
			_('Apply rate limit to µTP protocol.'));
		o.enabled = 'true';
		o.disabled = 'false';
		o.default = o.enabled;
